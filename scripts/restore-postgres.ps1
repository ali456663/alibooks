...
if ($PgDatabase -notmatch "(?i)(test|restore|drill)") {
  throw "Refusing restore: PGDATABASE must look like a test/restore/drill database. Use a separate database such as alibooks_restore_test."
}
...
docker run --rm `
  ...
  -v "${restoreDirAbs}:/backups" `
  postgres:16 `
  pg_restore -h $PgHost -p $PgPort -U $PgUser -d $PgDatabase --clean --if-exists --no-owner --no-privileges "/backups/$restoreFileBase"
