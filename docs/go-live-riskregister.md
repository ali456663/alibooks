...
| Fakturadokument och utskicksatervinning | BLOCKERAR SKARP DRIFT | Nya utstallda fakturor har immutable snapshot och original-PDF med kontrollsumma. Ett bestandigt leveransforsok sparas nu och markeras `SENT` eller `UNCERTAIN`, men automatisk outbox/aterforsok och SMTP/databas-commit ar fortfarande inte atomiska. Aldre dokument kan sakna original. | Bevara originalunderlag, granska `UNCERTAIN`-forsok och bygg bestaende outbox/atervinningsflode fore skarp drift. Se invoice-document-snapshots.md och release-evidence.md. | Originaldokument kan aterlasas oforandrade och varje SMTP-timeout/commitfel kan avstammas utan blinda dubbla utskick. |
